...
export class PerformanceTracker {
   ...
   */
  async loadPriceData(ticker: string, fromDate: string, toDate: string) {
    ...
    const cached: any = await dbStore.getPriceCache(ticker, fromDate, toDate);
   ...
   */
  async loadCurrentPrices(tickers: string[]) {
   ...
   */
  async calculatePortfolioValue(portfolio: any, date: string) {
    let totalValue = 0;
   ...
   */
  async calculatePnL(portfolio: any) {
    const currentPrices = await this.loadCurrentPrices(portfolio.positions.map((p: any) => p.ticker));
    ...
    const totalValue = positions.reduce((sum: number, p: any) => sum + p.current_value, 0);
    const totalCost = positions.reduce((sum: number, p: any) => sum + p.cost_basis, 0);
   ...
   */
  async calculateEquityCurve(portfolio: any, fromDate: string | null = null, toDate: string | null = null) {
    const from = fromDate || portfolio.created_at.split('T')[0];
    const to = toDate || new Date().toISOString().split('T')[0];

    // Get snapshots from database
    const snapshots: any = await dbStore.getSnapshots(portfolio.id, from, to);
   ...
   */
  calculateDrawdowns(equityCurve: any[]) {
    const drawdowns = [];
   ...
   */
  calculateMaxDrawdown(equityCurve: any[]) {
   ...
   */
  calculatePerformanceMetrics(equityCurve: any[], riskFreeRate = 0.02) {
    if (equityCurve.length < 2) {
      return null;
    }

    const returns = equityCurve.map((p: any) => p.daily_return_pct / 100);
    ...
    const downsideVariance = negativeReturns.length > 0
      ? negativeReturns.reduce((sum: number, r: number) => sum + Math.pow(r, 2), 0) / negativeReturns.length
      : 0;
   ...
   */
  async calculateRiskMetrics(portfolio: any, initialHistoricalData: any[] | null = null) {
    // If historical data not provided, fetch it
    let historicalData: any[] | null = initialHistoricalData;
    if (!historicalData) {
      historicalData = [];
      ...
      for (const position of portfolio.positions) {
        ...
        historicalData!.push({
          ...
          prices: prices.map((p: any) => ({ date: p.date, close: p.price }))
        });
      }
    }
   ...
   */
  async compareToBenchmark(portfolio: any, portfolioEquity: any) {
    ...
    const benchmarkEquity = benchmarkPrices.map((p: any) => ({
      date: p.date,
   ...
   * @private
   */
  _findClosestPrice(prices: any, targetDate: any) {
   ...
   * @private
   */
  _generateDateRange(fromDate: any, toDate: any) {
    const dates = [];
   ...
   * @private
   */
  _calculateAlphaBeta(portfolioEquity: any, benchmarkEquity: any) {
    // Align dates
   ...
   * @private
   */
  _calculateTrackingError(portfolioEquity: any, benchmarkEquity: any) {
    const aligned = this._alignEquityCurves(portfolioEquity, benchmarkEquity);
   ...
   * @private
   */
  _alignEquityCurves(portfolioEquity: any, benchmarkEquity: any) {
    const aligned = [];

    for (const p of portfolioEquity) {
      const b = benchmarkEquity.find((b: any) => b.date === p.date);
      if (b) {
        aligned.push({
          date: p.date,
          portfolio: p.value,
