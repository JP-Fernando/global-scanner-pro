 ...
 */
const calculateSectorWeights = (portfolio) => {
  const sectorWeights = {};
  let totalWeight = 0;

  portfolio.forEach(position => {
    const sector = position.sector || 999;
    const weight = parseFloat(position.current_weight || position.weight || 0);
 ...
 */
export const runSectorStressTest = (portfolio, totalCapital, scenario) => {
 ...
 * @returns {Object} Stress test results
 */
export const runCurrencyStressTest = (portfolio, totalCapital, scenario) => {
  let totalImpact = 0;
  const positionImpacts = [];
 ...
 * @returns {Object} Stress test results
 */
export const runGeopoliticalStressTest = (portfolio, totalCapital, scenario) => {
  let totalLoss = 0;
  const positionImpacts = [];
 ...
 * @returns {Object} Stress test results
 */
export const runLiquidityStressTest = (portfolio, totalCapital, scenario) => {
  let totalImpact = 0;
 ...
 */
export const runMultiFactorStressTest = (portfolio, totalCapital) => {
  ...
  const worstCase = allTests.reduce((worst, test) => {
    ...
    return loss > parseFloat(worst.totalLoss || 0) ? test : worst;
  }, allTests[0]);
  ...
  return {
    summary: {
      ...
      worstCaseValue: worstCase.newPortfolioValue || worstCase.liquidationValue
    },
