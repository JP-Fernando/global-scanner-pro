...
interface MCScenario {
  ...
  shocks: {
    market?: number;
    sectors?: Record<string | number, number>;
  };
}

interface MCPositionImpact {
  ticker: string;
...
interface MCScenarioResult {
  ...
  positionImpacts: MCPositionImpact[];
}
 ...
 */
const calculatePortfolioStats = (positions: MCPosition[]) => {
  const returns = [];

  // Calculate daily portfolio returns
  const maxLength = Math.max(...positions.map((p: MCPosition) => p.prices?.length || 0));

  for (let i = 1; i < maxLength; i++) {
    let portfolioReturn = 0;
    let totalWeight = 0;

    positions.forEach((position: MCPosition) => {
      if (position.prices && position.prices[i] && position.prices[i-1]) {
        ...
        const weight = parseFloat(String(position.weight || position.current_weight || 0));
 ...
 */
export const runMonteCarloSimulation = (positions: MCPosition[], initialCapital: number, options: MCSimulationOptions = {}) => {
  const {
 ...
 */
export const runHistoricalScenarios = (positions: MCPosition[], initialCapital: number, scenarios: MCScenario[]) => {
  const results: MCScenarioResult[] = [];

  scenarios.forEach((scenario: MCScenario) => {
    const { name, description, startDate, endDate, shocks } = scenario;

    let totalImpact = 0;
    const positionImpacts: MCPositionImpact[] = [];
