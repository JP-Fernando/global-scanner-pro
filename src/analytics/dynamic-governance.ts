...
interface VolatilityRegimeEntry {
  name: string;
  threshold: number;
  multiplier: number;
}

interface CorrelationRegimeEntry {
  name: string;
  threshold: number;
  multiplier: number;
}

interface DetectedVolatilityRegime extends VolatilityRegimeEntry {
  current_vol: number;
}

interface DetectedCorrelationRegime extends CorrelationRegimeEntry {
  current_corr: number;
}
...
interface MonitorResult {
  current_limits: DynamicLimitsResult;
  alerts: Alert[];
  timestamp: string;
}
...
interface StressTestResult {
  scenario: string;
  conditions: MarketConditions;
  adjusted_limits: InvestmentRules;
  metadata: AdjustmentMetadata;
}

interface AdjustedProfileResult {
  original_profile: string;
  adjusted_rules: InvestmentRules;
...
export const VOLATILITY_REGIMES: Record<string, VolatilityRegimeEntry> = {
...
export const CORRELATION_REGIMES: Record<string, CorrelationRegimeEntry> = {
 ...
 */
export function detectVolatilityRegime(portfolioVolatility: number): DetectedVolatilityRegime {
 ...
 */
export function detectCorrelationRegime(avgCorrelation: number): DetectedCorrelationRegime {
 ...
 */
export function calculateAverageCorrelation(correlationMatrix: number[][]): number {
...
export function calculateDynamicLimits(
  ...
  baseRules: InvestmentRules = INVESTMENT_RULES as InvestmentRules
): DynamicLimitsResult {
  const {
  ...
  const dynamicRules: InvestmentRules = {
    ...baseRules,
  ...
  const adjustmentMetadata: AdjustmentMetadata = {
    regime: {
...
function generateRecommendation(
  volRegime: DetectedVolatilityRegime,
  corrRegime: DetectedCorrelationRegime,
  stressLevel: number
): Recommendation[] {
 ...
 */
export function adjustRiskProfile(
  baseProfile: string,
  marketConditions: MarketConditions
): AdjustedProfileResult {
  const baseRules = (RISK_PROFILES as unknown as Record<string, { rules: InvestmentRules }>)[baseProfile]?.rules
    || INVESTMENT_RULES as InvestmentRules;
 ...
 */
function determineEffectiveProfile(rules: InvestmentRules): string {
 ...
 */
export function stressTestDynamicLimits(
  baseRules: InvestmentRules = INVESTMENT_RULES as InvestmentRules
): StressTestResult[] {
  ...
  const results: StressTestResult[] = scenarios.map((scenario: StressTestScenario) => {
 ...
 */
function generateHighCorrMatrix(size: number, avgCorr: number): number[][] {
  const matrix: number[][] = [];
 ...
 */
export function monitorMarketConditions(
  currentConditions: MarketConditions,
  historicalConditions: MarketConditions[] = []
): MonitorResult {
  ...
  const investmentRules = INVESTMENT_RULES as InvestmentRules;
  const positionReduction = (
    (investmentRules.max_position_weight - current.rules.max_position_weight) /
    investmentRules.max_position_weight
  ) * 100;
