...
export class AttributionAnalyzer {
   ...
   */
  calculateAttribution(portfolio: any, portfolioReturns: any, benchmarkReturns: any, factorScores: any = null) {
   ...
   * @private
   */
  _calculateBrinsonAttribution(portfolio: any, portfolioReturns: any, benchmarkReturns: any) {
    // Group positions by sector
    const sectorGroups = this._groupPositionsBySector(portfolio.positions);

    // Calculate sector returns for portfolio
    const portfolioSectorReturns: { [key: string]: any } = this._calculateSectorReturns(
      sectorGroups,
      portfolioReturns
    );
    ...
    const benchmarkSectorWeights: { [key: string]: any } = this._estimateBenchmarkSectorWeights(portfolio?.benchmark);
    ...
    const allocationEffect: any[] = [];
    const selectionEffect: any[] = [];
    const interactionEffect: any[] = [];
    ...
    for (const [sectorId, positions] of Object.entries(sectorGroups)) {
      ...
      const portfolioWeight = (positions as any[]).reduce((sum: any, p: any) => sum + (p.current_weight || 0), 0);
    ...
    const totalAllocation = allocationEffect.reduce((sum: any, e: any) => sum + e.contribution, 0);
    const totalSelection = selectionEffect.reduce((sum: any, e: any) => sum + e.contribution, 0);
    const totalInteraction = interactionEffect.reduce((sum: any, e: any) => sum + e.contribution, 0);

    const sortByAbsContribution = (a: any, b: any) => Math.abs(b.contribution) - Math.abs(a.contribution);
   ...
   * @private
   */
  _calculateFactorAttribution(portfolio: any, portfolioReturns: any, factorScores: any) {
    const factorContributions: { trend: any[]; momentum: any[]; risk: any[]; liquidity: any[] } = {
      trend: [],
      momentum: [],
      risk: [],
      liquidity: []
    ...
    // Calculate totals
    const sumContribs = (arr: any[]) => arr.reduce((sum: any, e: any) => sum + e.contribution, 0);
    ...
    const sortAndSlice = (arr: any[]) => arr
    ...
    const calcPct = (val: any) => (val / totalSum * 100) || 0;
   ...
   * @private
   */
  _calculateAssetContribution(portfolio: any, portfolioReturns: any) {
    const contributions = portfolio.positions.map((position: any) => {
      const positionReturn = this._calculatePositionReturn(position, portfolioReturns);
      const weight = position.current_weight || 0;
      const contribution = positionReturn * weight;
    ...
    contributions.sort((a: any, b: any) => Math.abs(b.contribution) - Math.abs(a.contribution));

    const totalContribution = contributions.reduce((sum: any, c: any) => sum + c.contribution, 0);

    return {
      total_contribution: totalContribution,
      top_contributors: contributions.slice(0, 10),
      top_detractors: contributions.filter((c: any) => c.contribution < 0).slice(0, 5),
   ...
   * @private
   */
  _calculatePeriodAttribution(portfolioReturns: any, benchmarkReturns: any) {
    const periods: { monthly: any[]; quarterly: any[]; yearly: any[] } = {
      monthly: [],
      quarterly: [],
      yearly: []
    };
    ...
    const benchmarkMonthlyGroups: { [key: string]: any } = this._groupByPeriod(benchmarkReturns, 'month');

    for (const [period, returns] of Object.entries(monthlyGroups)) {
      const portfolioReturn = this._calculatePeriodReturn(returns as any[]);
      ...
      periods.monthly.push({
        ...
        days: (returns as any[]).length
      });
    }

    // Quarterly attribution
    const quarterlyGroups = this._groupByPeriod(portfolioReturns, 'quarter');
    const benchmarkQuarterlyGroups: { [key: string]: any } = this._groupByPeriod(benchmarkReturns, 'quarter');
    ...
    const benchmarkYearlyGroups: { [key: string]: any } = this._groupByPeriod(benchmarkReturns, 'year');
   ...
   */
  calculateEventAttribution(portfolioReturns: any, benchmarkReturns: any, events: any) {
    const eventAnalysis = events.map((event: any) => {
    ...
    const avgExcessReturn = eventAnalysis.reduce((s: any, e: any) => s + e.excess_return, 0) / events.length;

    return {
      events: eventAnalysis,
      summary: {
        total_events: events.length,
        outperformed: eventAnalysis.filter((e: any) => e.excess_return > 0).length,
        underperformed: eventAnalysis.filter((e: any) => e.excess_return < 0).length,
   ...
   * @private
   */
  _calculateTotalReturn(returns: any) {
   ...
   * @private
   */
  _groupPositionsBySector(positions: any) {
   ...
   * @private
   */
  _calculateSectorReturns(sectorGroups: any, portfolioReturns: any) {
  ...
  _estimateBenchmarkSectorWeights(benchmark: any) {
   ...
   * @private
   */
  _getSectorName(sectorId: any) {
    const sector = SECTOR_TAXONOMY.find((s: any) => s.sectorId === sectorId);
   ...
   * @private
   */
  _calculatePositionReturn(position: any, portfolioReturns: any) {
   ...
   * @private
   */
  _getCurrentPrice(position: any, portfolioReturns: any) {
    ...
    if (latestSnapshot && latestSnapshot.positions) {
      const positionData = latestSnapshot.positions.find((p: any) => p.ticker === position.ticker);
      if (positionData) {
   ...
   * @private
   */
  _groupByPeriod(returns: any, periodType: any) {
    const groups: { [key: string]: any[] } = {};

    returns.forEach((point: any) => {
   ...
   * @private
   */
  _calculatePeriodReturn(periodReturns: any) {
   ...
   * @private
   */
  _getReturnsInPeriod(returns: any, startDate: any, endDate: any) {
    const start = new Date(startDate);
    const end = new Date(endDate);

    return returns.filter((point: any) => {
      const date = new Date(point.date);
   ...
   * @private
   */
  _calculateMaxDrawdownInPeriod(returns: any) {
    if (!returns || returns.length < 2) return 0;
   ...
   * @private
   */
  _calculateSharpeRatio(returns: any, riskFreeRate = 0.02) {
    if (!returns || returns.length < 2) return 0;
    ...
    const avgReturn = dailyReturns.reduce((sum: any, r: any) => sum + r, 0) / dailyReturns.length;
    const sumSquaredDiff = dailyReturns.reduce((sum: any, r: any) => sum + Math.pow(r - avgReturn, 2), 0);
   ...
   * @private
   */
  _interpretBrinsonResults(allocationEffect: any, selectionEffect: any) {
