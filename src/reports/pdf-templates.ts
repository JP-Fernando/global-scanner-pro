 ...
 */
export function generateAuditReport(portfolio: any, governance: any, riskData: any, performanceData: any) {
  ...
  if (governance && governance.compliance) {
    ...
    if (governance.compliance.issues && governance.compliance.issues.length > 0) {
      ...
      const issuesTable = governance.compliance.issues.map((issue: any) => [
        issue.level || 'MEDIUM',
        issue.rule || '',
        issue.description || '',
  ...
  if (portfolio.positions && portfolio.positions.length > 0) {
    const positionsTable = portfolio.positions.map((p: any) => [
      p.ticker || '',
      p.quantity || '',
  ...
  if (portfolio.rebalanceHistory && portfolio.rebalanceHistory.length > 0) {
    ...
    const rebalanceTable = portfolio.rebalanceHistory.slice(0, 10).map((r: any) => [
...
export function generateInvestmentCommitteeReport(
  portfolio: any, performanceData: any, riskData: any, marketContext: any
) {
 ...
 */
export function generateClientReport(portfolio: any, performanceData: any, riskData: any) {
 ...
 */
export function generateAttributionReport(portfolio: any, attributionData: any) {
  ...
  if (attributionData?.brinson?.allocation_effect?.by_sector?.length) {
    const brinsonRows = attributionData.brinson.allocation_effect.by_sector.map((sector: any) => [
      sector.sector,
  ...
  if (attributionData?.assets?.top_contributors?.length) {
    const assetRows = attributionData.assets.top_contributors.map((asset: any) => [
      asset.ticker,
      asset.name,
      asset.sector,
  ...
  if (attributionData?.periods?.monthly?.length) {
    const periodRows = attributionData.periods.monthly.slice(-12).reverse().map((period: any) => [
      period.period,
  ...
  if (attributionData?.events?.events?.length) {
    const eventRows = attributionData.events.events.map((event: any) => [
      event.event_name,
 ...
 */
export function generateBacktestPDF(results: any, _options = {}) {
  ...
  const perfTable = results.map((r: any) => [
    r.strategyName || '',
