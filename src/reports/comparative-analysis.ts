...
interface PeriodComparison {
  period1?: { label?: string; cagr?: number; sharpeRatio?: number; maxDrawdown?: number; volatility?: number };
  period2?: { label?: string; cagr?: number; sharpeRatio?: number; maxDrawdown?: number; volatility?: number };
  differences?: { cagr?: number; sharpeRatio?: number };
  winner?: string | null;
}
 ...
 */
export function compareBacktestStrategies(results: DatasetItem[]) {
 ...
 */
export function comparePerformancePeriods(performanceData: DatasetItem[], periods: Array<{ label: string; start: string; end: string }>) {
 ...
 */
export function generateComparativePDF(datasets: DatasetItem[], title: string = 'Comparative Analysis') {
  const pdf = new PDFReportGenerator(datasets as any);
  ...
  const summaryTable = datasets.map((d: DatasetItem) => [
  ...
  datasets.forEach((dataset: DatasetItem, idx: number) => {
 ...
 */
export function generateComparativeExcel(datasets: DatasetItem[], _title: string = 'Comparative Analysis') {
  const excel = new ExcelReportGenerator(datasets as any);
 ...
 */
export function compareTwoPeriods(periodData1: PeriodData, periodData2: PeriodData, labels: string[] = ['Period 1', 'Period 2']) {
  const comparison: {
    period1: PeriodData & { label: string };
    period2: PeriodData & { label: string };
    differences: { cagr: number; sharpeRatio: number; maxDrawdown: number; volatility: number };
    winner: string | null;
  } = {
    period1: {
      label: labels[0],
  ...
  if ((periodData1.sharpeRatio || 0) > (periodData2.sharpeRatio || 0)) {
    comparison.winner = labels[0];
  } else if ((periodData2.sharpeRatio || 0) > (periodData1.sharpeRatio || 0)) {
 ...
 */
export function generatePeriodComparisonPDF(periodComparisons: PeriodComparison[]) {
  const pdf = new PDFReportGenerator(periodComparisons as any);
  ...
  // Comparison table
  const compTable = periodComparisons.map((p: PeriodComparison) => [
  ...
  compTable.push(
    ...periodComparisons.map((p: PeriodComparison) => [
  ...
  periodComparisons.forEach((comp: PeriodComparison) => {
