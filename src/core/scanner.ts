...
let mlRegimePrediction: any = null;
...
const SECTOR_NAMES: Record<string | number, string> = SECTOR_TAXONOMY.reduce((acc: Record<string | number, string>, sector) => {
...
async function loadYahooData(ticker: string, suffix: string): Promise<any[] | null> {
  ...
  try {
    ...
    const series = r.timestamp.map((t: any, i: any) => {
...
async function analyzeStock(stock: any, suffix: string, config: any, benchmarkROCs: any, benchmarkVol: any): Promise<any> {
  try {
...
async function loadBenchmark(suffix: string) {
  const benchmarkSymbol = (MARKET_BENCHMARKS as any)[suffix];
  ...
  try {
    ...
    console.warn(i18n.t('errors.benchmark_calculation_failed'), (e as Error).message);
    return null;
  }
}

function getRSIDescription(rsi: number) {
  if (rsi >= 70) {
    return {
...
function updateSectorUI(sectorStats: any) {
  const container = document.getElementById('sectorSummary');
  if (!container) return;

  // Clear and generate cards
  container.innerHTML = Object.entries(sectorStats).map(([id, stat]: [string, any]) => {
    const dotColor = (SECTOR_COLORS as any)[id] || (SECTOR_COLORS as any)[999];
    ...
    return `
      <div style="background:#1e293b; padding:15px; border-radius:12px; border-left: 5px solid ${dotColor}; border: 1px solid #334155; transition: transform 0.2s;">
        ...
        <div style="margin-top:12px; display:flex; justify-content:space-between; align-items:center;">
          <span style="color:#475569; font-size:0.75em;">${(stat as any).count} assets</span>
          <div style="width:40px; height:4px; background:#334155; border-radius:2px;">
             <div style="width:${(stat as any).avgRsi}%; height:100%; background:${dotColor}; border-radius:2px;"></div>
          </div>
        </div>
      </div>
    `;
...
async function loadUniverseData(file: any, suffix: any, statusNode: any) {
  if (statusNode) {
  ...
  for (let i = 0; i < universe.length; i += BATCH_SIZE) {
    ...
    if (statusNode) {
      statusNode.innerText = i18n.t('status.downloading_historical', {
        current: String(i + 1),
        end: String(Math.min(i + BATCH_SIZE, universe.length)),
        total: String(universe.length)
      });
    }
...
function formatPct(value: any, { showSign = true } = {}) {
...
function getMetricColor(value: any, thresholds: any) {
...
function toggleSection(sectionId: any, el: any) {
...
function renderBacktestResults(results: any, rebalanceEvery: any, benchmarkReturns = null) {
  const container = document.getElementById('backtestResults');
  if (!container) return;

  const validResults = results.filter((r: any) => r.metrics);
  validResults.sort((a: any, b: any) => (b.metrics.sharpeRatio ?? 0) - (a.metrics.sharpeRatio ?? 0));
...
function renderBacktestHeader(
  results: any, rebalanceEvery: any, initialCapital = lastBacktestInitialCapital
) {
  const sumSharpe = results.reduce((sum: any, r: any) => sum + (r.metrics?.sharpeRatio || 0), 0);
  const avgSharpe = sumSharpe / results.length;
  const sumCAGR = results.reduce((sum: any, r: any) => sum + (r.metrics?.cagr || 0), 0);
...
function renderPerformanceComparison(results: any) {
  return `
...
function renderDetailedMetrics(results: any) {
  return `
    <div style="background: #1e293b; padding: 25px; border-radius: 12px; margin-bottom: 20px;">
...
function renderRiskMetrics(results: any) {
  return `
...
function renderTradingMetrics(results: any) {
  return `
...
function renderEquityCurveChart(bestStrategy: any, benchmarkReturns = null) {
  if (!bestStrategy || !bestStrategy.equityCurve) return '';

  const curve = bestStrategy.equityCurve;
  if (curve.length < 2) return '';
  let benchmarkCurve: any = null;

  if (benchmarkReturns && (benchmarkReturns as any).length > 0) {
    benchmarkCurve = [1];
    (benchmarkReturns as any).forEach((ret: any) => {
  ...
  const strategyPoints = alignedCurve.map((val: any, idx: any) => {
  ...
  let benchmarkPoints = '';
  if (alignedBenchmark) {
    benchmarkPoints = alignedBenchmark.map((val: any, idx: any) => {
  ...
  const drawdownPoints = alignedCurve.map((val: any, idx: any) => {
...
function renderDrawdownAnalysis(results: any) {
  return `
...
(window as any).renderBacktestResults = renderBacktestResults;
(window as any).toggleSection = toggleSection;
(window as any).exportBacktestToCSV = exportBacktestToCSV;
(window as any).exportBacktestToExcelAdvanced = exportBacktestToExcelAdvanced;
(window as any).exportBacktestToPDFAdvanced = exportBacktestToPDFAdvanced;
(window as any).exportBacktestComparative = exportBacktestComparative;
(window as any).exportScanResults = exportScanResults;

(window as any).runBacktest = async function () {
  const [file, suffix] = (document.getElementById('marketSelect') as HTMLInputElement)!.value.split('|');
  const topN = parseInt((document.getElementById('backtestTopN') as HTMLInputElement)!.value, 10);
  const rebalanceEvery = parseInt((document.getElementById('backtestRebalance') as HTMLInputElement)!.value, 10);
  const allocationMethod = (document.getElementById('backtestAllocationMethod') as HTMLInputElement)!.value;
  const initialCapitalValue = parseFloat((document.getElementById('backtestInitialCapital') as HTMLInputElement)!.value);
  ...
  try {
    ...
    status!.innerText = `${i18n.t('errors.backtest_failed')  }: ${  (err as Error).message}`;
  }
};
...
async function showAllMarketsWarning(marketCount: any) {
  return new Promise((resolve) => {
    ...
    const proceedBtn = content.querySelector('#proceedBtn') as HTMLElement | null;
    const cancelBtn = content.querySelector('#cancelBtn') as HTMLElement | null;
    ...
    cancelBtn!.addEventListener('mouseenter', () => {
      (cancelBtn as HTMLElement).style.background = '#475569';
    });

    cancelBtn!.addEventListener('mouseleave', () => {
      (cancelBtn as HTMLElement).style.background = '#64748b';
    });
...
function getAllMarketOptions() {
  ...
  for (const option of (marketSelect as HTMLSelectElement).options) {
...
async function scanSingleMarket(file: any, suffix: any, config: any, status: any, isPartOfAllMarkets = false) {
...
export async function runScan() {
  ...
  try {
    ...
    if (btnScan) {
      ...
      (btnScan as HTMLElement).innerText = i18n.t('status.scanning');
      btnScan.style.opacity = '0.7';
      btnScan.style.cursor = 'wait';
    }

    const marketValue = (document.getElementById('marketSelect') as HTMLInputElement)!.value;
    const strategyKey = (document.getElementById('strategySelect') as HTMLInputElement)!.value;
    const config = (STRATEGY_PROFILES as Record<string, any>)[strategyKey];
    ...
    if (marketValue === 'ALL_MARKETS') {
      ...
      for (let i = 0; i < allMarkets.length; i++) {
        ...
        const progressMessage = i18n.t('status.scanning_market', {
          current: String(i + 1),
          total: String(allMarkets.length),
          market: market.name
        });
      ...
      currentResults = currentResults.map(asset => {
        ...
        if (anomalyData.hasAnomalies) {
          penalty = Math.min(15, Math.round(Number(anomalyData.metrics?.volumeZScore || 0) * 3));
      ...
      status!.innerText = i18n.t('status.all_markets_complete', { count: String(totalAnalyzed) });
      filterInfo!.innerHTML = i18n.t('filters.info', {
        approved: String(totalAnalyzed),
        filtered: String(totalFiltered)
      });
      ...
      const canBuildPortfolio =
        appState.scanResults.filter(
          (r: any) => Array.isArray(r.pricesWithDates) && r.pricesWithDates.length >= 30
        ).length >= 3;

      const buildBtn = document.querySelector('button[onclick="buildPortfolio()"]');
      if (buildBtn) {
        (buildBtn as HTMLButtonElement).disabled = !canBuildPortfolio;
      }
    ...
    status!.innerText = i18n.t('status.scan_complete', { count: String(analyzed) });
    filterInfo!.innerHTML = i18n.t('filters.info', {
      approved: String(analyzed),
    ...
    if (!performanceTracker && appState.mlEnabled) {
      try {
        ...
        console.warn('Performance Tracker initialization failed, creating new:', (e as Error).message);
    ...
    if (benchmarkData && benchmarkData.symbol) {
      try {
        ...
        if (!benchmarkFullData || benchmarkFullData.length === 0) {
          ...
          const benchmarkPrices = benchmarkFullData.map((d: any) => ({ date: d.date, close: d.close }));
          if (benchmarkPrices.length >= 200) {
            // Rule-based regime detection
            currentRegime = regime.detectMarketRegime(benchmarkPrices as any, currentResults);
            ...
            if (appState.mlEnabled) {
              try {
                ...
                const regimeFeatures = extractRegimeFeatures(benchmarkPrices as any);
                ...
                console.warn('ML Regime prediction failed:', (e as Error).message);
              }
            }
    ...
    if (appState.mlEnabled && performanceTracker && currentRegime) {
      try {
        ...
        console.warn('ML Adaptive scoring failed:', (e as Error).message);
      }
    }
    ...
    if (appState.mlEnabled && currentResults.length > 0) {
      try {
        ...
        console.warn('ML Anomaly detection failed:', (e as Error).message);
      }
    }
    ...
    if (appState.mlEnabled && currentResults.length > 0) {
      try {
        ...
        console.warn('ML Recommendations generation failed:', (e as Error).message);
      }
    }
    ...
    if (appState.mlEnabled) {
      try {
        ...
        console.warn('ML UI rendering failed:', (e as Error).message);
      }
    }
    ...
    if (btnScan) {
      ...
      (btnScan as HTMLElement).innerText = i18n.t('buttons.runScan');
...
(window as any).buildPortfolio = function () {
  const method = (document.getElementById('allocationMethod') as HTMLInputElement)!.value;
  const topN = parseInt((document.getElementById('topNAssets') as HTMLInputElement)!.value, 10);
  const totalCapital = parseFloat((document.getElementById('totalCapital') as HTMLInputElement)!.value);
  const riskProfile = (document.getElementById('riskProfile') as HTMLInputElement)?.value || 'moderate';
  ...
  try {
    ...
    let withCapital: any[] = allocation.calculateCapitalRecommendations(
    ...
    const enrichedAllocation = withCapital.map(asset => {
      const original = appState.scanResults.find((r: any) => r.ticker === asset.ticker);
      return {
        ...asset,
    ...
    showInlineError(`Error construyendo cartera: ${(err as any).message}`);
  }
};
...
function renderPortfolio(portfolio: any) {
...
function riskCard(label: any, value: any) {
  return `
    <div class="risk-card">
...
function renderAdvancedRiskDashboard(riskReport: any) {
  return `
    <div class="risk-dashboard-container">
      ...
      <div style="margin-top: 20px;">
        ...
        <table class="stress-table">
          ...
          <tbody>
            ${riskReport.stressTests.map((s: any) => `
              <tr>
...
function renderHeatmap(matrix: any) {
  const tickers = matrix.map((m: any) => m.ticker);

  let html = '<table class="heatmap-table"><thead><tr><th></th>';
  tickers.forEach((t: any) => html += `<th>${t.split('.')[0]}</th>`);
  html += '</tr></thead><tbody>';

  matrix.forEach((row: any, _i: any) => {
    html += `<tr><td><strong>${row.ticker.split('.')[0]}</strong></td>`;
    row.values.forEach((val: any, _j: any) => {
...
function renderAllocationTable(allocation: any) {
  return `
...
function renderWeightChart(allocation: any) {
  const top = allocation.slice(0, 10);
  const maxWeight = Math.max(...top.map((a: any) => parseFloat(a.weight_pct)));
...
function getFilterState() {
  ...
  return {
    ...
    signal: (signalFilter as HTMLSelectElement)?.value || 'all',
    minScore: Number((minScoreInput as HTMLInputElement)?.value || 0),
    volume: (volumeFilter as HTMLSelectElement)?.value || 'all'
  };
}

function updateFilterSummary(filteredCount: any, totalCount: any) {
...
function applyFilters() {
  ...
  const viewKey = (document.getElementById('viewMode') as HTMLInputElement)?.value || 'scoreTotal';
...
function resetFilters() {
  ...
  if (signalFilter) (signalFilter as HTMLSelectElement).value = 'all';
  if (minScoreInput) (minScoreInput as HTMLInputElement).value = '0';
  if (volumeFilter) (volumeFilter as HTMLSelectElement).value = 'all';
...
// Renderizar régimen
function renderRegimeIndicator(regimeData: any) {
    ...
    <div style="background: #1e293b; padding: 20px; border-radius: 12px; margin: 20px 0; border-left: 4px solid #818cf8;">
      <h3 style="color: #818cf8; margin-bottom: 15px; display: flex; align-items: center; gap: 10px;">
        ...
        <span style="font-size: 0.7em; background: #334155; padding: 4px 12px; border-radius: 12px; color: #94a3b8;">
          ${i18n.t('ml.recommendations.insights_count', { count: String(mlRecommendations.length) })}
        </span>
      </h3>
      <div style="display: grid; gap: 12px;">
        ${mlRecommendations.slice(0, 10).map((rec: any) => `
          <div style="background: #0f172a; padding: 15px; border-radius: 8px; border-left: 3px solid ${(priorityColors as any)[rec.priority.level]};">
            <div style="display: flex; align-items: start; gap: 10px; margin-bottom: 8px;">
              <span style="font-size: 1.5em;">${(priorityIcons as any)[rec.priority.level]}</span>
              <div style="flex: 1;">
                <div style="font-weight: bold; color: ${(priorityColors as any)[rec.priority.level]}; margin-bottom: 5px;">
  ...
  const html = `
    <div style="background: #1e293b; padding: 20px; border-radius: 12px; margin: 20px 0; border-left: 4px solid #f43f5e;">
      ...
      ${summary.by_severity ? `
        <div style="display: flex; gap: 10px; margin-bottom: 15px; flex-wrap: wrap;">
          ${Object.entries(summary.by_severity).map(([severity, count]) => `
            <div style="background: #0f172a; padding: 8px 12px; border-radius: 6px; border: 1px solid ${(severityColors as any)[severity]};">
              <span style="font-size: 0.8em; color: #94a3b8;">${i18n.t(`ml.anomalies.severity_${severity}`)}:</span>
              <strong style="color: ${(severityColors as any)[severity]}; margin-left: 5px;">${count}</strong>
            </div>
          `).join('')}
        </div>
      ` : ''}
      ...
      <div style="display: grid; gap: 10px; max-height: 400px; overflow-y: auto;">
        ${mlAnomalies.slice(0, 15).map((anomaly: any, idx: any) => `
          <div onclick="showAnomalyDetails(${idx})" style="background: #0f172a; padding: 12px; border-radius: 6px; border-left: 3px solid ${(severityColors as any)[anomaly.severity]}; cursor: pointer; transition: all 0.2s ease;" onmouseover="this.style.background='#1e293b'" onmouseout="this.style.background='#0f172a'">
            <div style="display: flex; align-items: center; gap: 8px; margin-bottom: 5px; justify-content: space-between;">
              <div style="display: flex; align-items: center; gap: 8px;">
                <span>${(severityIcons as any)[anomaly.severity]}</span>
                <strong style="color: #f8fafc;">${anomaly.ticker || 'N/A'}${anomaly.name ? ` (${anomaly.name})` : ''}</strong>
                <span style="color: ${(severityColors as any)[anomaly.severity]}; font-size: 0.85em; font-weight: 600;">
...
function generateTimeHorizonRecommendations(result: any) {
  ...
  const getColorForScore = (score: any) => {
 ...
 */
function checkMLAnomaliesForTicker(ticker: any) {
  ...
  const maxSeverity = relevantAnomalies.reduce((max, a) => {
    return ((severityOrder as any)[a.severity] || 0) > ((severityOrder as any)[max] || 0) ? a.severity : max;
  }, 'low');
 ...
 */
function generateInvestmentRecommendation(result: any, options: any = {}): any {
  ...
  if (options.marketData || options.allAssets) {
    try {
      ...
      console.warn('ML analysis failed for', result.ticker, (e as any).message);
    }
  }
  ...
  // Undervalued opportunity
    ...
    params = {
      ...
      scoreMedium: scoreMedium.toFixed(0),
      scoreLong: scoreLong.toFixed(0),
      mlAnomalyWarning: hasMLAnomalies ? i18n.t('recommendation.ml_anomaly_detected', { count: String(mlAnomalyInfo.count), severity: mlAnomalySeverityLabel }) : ''
    };
 ...
 */
function generateMLInsightsHTML(mlInsights: any) {
  ...
  if (mlInsights.regimeImpact) {
    ...
    const impactColor = (impactColors as any)[ri.impact] || '#64748b';
  ...
  if (mlInsights.momentumShift && mlInsights.momentumShift.shift !== 'stable') {
    ...
    const shiftColor = (shiftColors as any)[ms.shift] || '#64748b';
  ...
  if (mlInsights.mlSignal) {
    ...
    const signalColor = (signalColors as any)[sig.signal] || '#64748b';
  ...
  if (mlInsights.riskScore) {
    ...
    const riskColor = (riskColors as any)[rs.riskLevel] || '#64748b';
...
function showDetails(result: any) {
  const modal = document.getElementById('detailModal');
  const content = document.getElementById('modalContent');
  const d = result.details;

  const li = (labelKey: any, valueHtml: any) =>
    `<li>${i18n.t(labelKey)}: <strong>${valueHtml}</strong></li>`;

  const scoreCell = (labelKey: any, value: any) =>
    `<div>${i18n.t(labelKey)}: <strong>${value}</strong></div>`;

  const timeCard = (labelKey: any, value: any, style: any) => `
    <div style="${style}">
  ...
  const mlAnomaliesSection = mlInfo.hasAnomalies ? `
    <div style="background: rgba(239, 68, 68, 0.1); padding: 15px; border-radius: 8px; border-left: 3px solid #ef4444; margin-top: 15px;">
      <h4 style="color: #ef4444; margin-bottom: 10px; font-size: 0.95em;">
        🤖 ${i18n.t('details.ml_anomalies_detected', { count: String(mlInfo.count) })}
      </h4>
...
function showAnomalyDetails(anomalyIndex: any) {
  const modal = document.getElementById('detailModal');
  const content = document.getElementById('modalContent');
  ...
  content!.innerHTML = `
    ...
    <div style="background: ${(severityColors as any)[anomaly.severity]}20; padding: 15px; border-radius: 8px; border-left: 4px solid ${(severityColors as any)[anomaly.severity]}; margin-bottom: 20px;">
      <div style="display: flex; align-items: center; gap: 10px; margin-bottom: 10px;">
        <strong style="color: #f8fafc; font-size: 1.2em;">${anomaly.ticker}${anomaly.name ? ` (${anomaly.name})` : ''}</strong>
        <span style="color: ${(severityColors as any)[anomaly.severity]}; font-size: 0.9em; font-weight: 600;">
...
(window as any).showAnomalyDetails = showAnomalyDetails;

// Render governance
function renderGovernanceReport(governanceReport: any, complianceCheck: any) {
    ...
    <div class="governance-report" style="background: #1e293b; padding: 25px; border-radius: 12px; margin-bottom: 30px; border-left: 4px solid ${statusColor};">
      ...
      ${hasViolations ? `
        <div style="background: rgba(244, 63, 94, 0.1); padding: 15px; border-radius: 8px; border-left: 3px solid #f43f5e;">
          ...
          <ul style="list-style: none; padding: 0; margin: 0;">
            ${complianceCheck.violations.map((v: any) => `
      ...
      ${complianceCheck.warnings.length > 0 ? `
        <div style="background: rgba(251, 191, 36, 0.1); padding: 15px; border-radius: 8px; border-left: 3px solid #fbbf24; margin-top: 15px;">
          ...
          <ul style="list-style: none; padding: 0; margin: 0;">
            ${complianceCheck.warnings.slice(0, 3).map((w: any) => `
...
(window as any).showRegimeDetails = function () {
...
function getRegimeInterpretation(regimeType: any) {
...
function showInlineError(message: any) {
  const status = document.getElementById('status');
...
function getSelectedOptionText(selectId: any) {
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (minScoreInput) {
    minScoreInput.addEventListener('input', () => {
      if (minScoreValue) {
        minScoreValue.textContent = (minScoreInput as HTMLInputElement).value;
      }
      applyFilters();
    });
  }
