...
const math = {
  ...
  std: (arr: number[], avg: number): number => {
...
interface AnomalyAsset {
  history?: Array<{ volume: number; close: number }>;
  volume: number;
  sectorId?: number;
}
...
export function detectAnomalies(
  asset: AnomalyAsset,
  sectorStats: Record<string | number, { avgVolume?: number }>,
  params: AnomalyParams = { thresholdZ: 3.0, sectorRatio: 5.0 }
  ...
  const sectorAvgVol = sectorStats[asset.sectorId ?? 999]?.avgVolume || avgVol;
