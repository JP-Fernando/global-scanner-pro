...
interface AssetDetails {
  momentum: { roc6m: string; roc12m: string; rsi: string; [key: string]: unknown };
  risk: { volatility: string; relativeVol: string; maxDrawdown: string; [key: string]: unknown };
  [key: string]: unknown;
}
...
interface MarketDataInput {
  ...
  assets?: AssetData[];
  volatility?: number;
  regime_prediction?: RegimePredictionInput;
  [key: string]: unknown;
}

interface RegimePredictionInput {
  regime: string;
  confidence: number;
...
interface HistoricalPerformanceRecord {
  return_60d: number;
  [key: string]: unknown;
}

interface HistoricalPerformanceMap {
  [ticker: string]: HistoricalPerformanceRecord;
}

interface RegimeImpactResult {
  regime: string;
  previous_regime: string;
  confidence: number;
  impact: string;
...
interface MLRiskScoreResult {
  riskLevel: string;
  riskScore: string;
  relativeRiskPercentile: string;
  volatility: string;
  maxDrawdown: string;
}

interface AssetMLInsights {
  regimeImpact: RegimeImpactResult | null;
  momentumShift: MomentumShiftResult | null;
  mlSignal: MLSignalResult | null;
  riskScore: MLRiskScoreResult | null;
}

export type SeverityKey = 'extreme' | 'high' | 'moderate' | 'low';
...
export const RECOMMENDATION_PRIORITY: Record<string, PriorityLevel> = {
 ...
 */
export function generateRecommendations(portfolio: PortfolioInput, marketData: MarketDataInput | null, historicalPerformance: HistoricalPerformanceMap | null): RecommendationItem[] {
...
function buildAnomalyMap(anomalies: AnomalyItem[] = []): Map<string, AnomalyItem[]> {
  const map = new Map<string, AnomalyItem[]>();

  anomalies.forEach((anomaly: AnomalyItem) => {
    const tickers = [anomaly.ticker, anomaly.ticker1, anomaly.ticker2].filter(Boolean) as string[];
...
function getMaxSeverity(anomalies: AnomalyItem[] = []): string {
  const severityOrder: Record<string, number> = { extreme: 4, high: 3, moderate: 2, low: 1 };

  return anomalies.reduce((max: string, anomaly: AnomalyItem) => {
...
function isSevereAnomaly(anomalies: AnomalyItem[] = []): boolean {
 ...
 */
function detectRebalancingNeeds(portfolio: PortfolioInput): RecommendationItem[] {
  const recommendations: RecommendationItem[] = [];
  const { positions, target_weights } = portfolio;

  if (!positions || !target_weights) return recommendations;

  Object.entries(positions).forEach(([ticker, position]: [string, PortfolioPosition]) => {
 ...
 */
function detectRiskWarnings(portfolio: PortfolioInput, marketData: MarketDataInput | null, anomalyMap: Map<string, AnomalyItem[]>): RecommendationItem[] {
  const recommendations: RecommendationItem[] = [];

  // Check concentration risk
  if (portfolio.positions) {
    const sortedPositions = (Object.values(portfolio.positions) as PortfolioPosition[])
      .sort((a: PortfolioPosition, b: PortfolioPosition) => b.weight - a.weight);

    const top3Concentration = sortedPositions.slice(0, 3)
      .reduce((sum: number, p: PortfolioPosition) => sum + p.weight, 0);
  ...
  if (marketData?.assets && anomalyMap && anomalyMap.size > 0) {
    ...
    marketData.assets.forEach((asset: AssetData) => {
 ...
 */
function detectBuyOpportunities(marketData: MarketDataInput | null, portfolio: PortfolioInput, anomalyMap: Map<string, AnomalyItem[]>): RecommendationItem[] {
  const recommendations: RecommendationItem[] = [];

  if (!marketData || !marketData.assets) return recommendations;

  // Find high-scoring assets not in portfolio
  const assetsNotOwned = marketData.assets.filter((asset: AssetData) => {
    return !portfolio.positions || !portfolio.positions[asset.ticker];
  });

  const topOpportunities = assetsNotOwned
    .filter((asset: AssetData) => asset.quant_score > 70)
    ...
    .sort((a: AssetData, b: AssetData) => b.quant_score - a.quant_score)
    .slice(0, 3);

  topOpportunities.forEach((asset: AssetData) => {
    recommendations.push({
 ...
 * Detect sell alerts
 */
function detectSellAlerts(portfolio: PortfolioInput, historicalPerformance: HistoricalPerformanceMap | null): RecommendationItem[] {
  const recommendations: RecommendationItem[] = [];

  if (!portfolio.positions || !historicalPerformance) return recommendations;

  Object.entries(portfolio.positions).forEach(([ticker, position]: [string, PortfolioPosition]) => {
 ...
 */
function detectDiversificationNeeds(portfolio: PortfolioInput): RecommendationItem[] {
  const recommendations: RecommendationItem[] = [];

  if (!portfolio.sector_exposure) return recommendations;

  // Check sector concentration
  Object.entries(portfolio.sector_exposure).forEach(([sector, weight]: [string, number]) => {
    if (weight > 0.35) {
      recommendations.push({
 ...
 */
function detectRegimeChanges(marketData: MarketDataInput | null): RecommendationItem[] {
  const recommendations: RecommendationItem[] = [];
 ...
 */
export function analyzeAssetML(result: AssetData, marketData: MarketDataInput | null, allAssets: AssetData[] | null): AssetMLInsights {
  const insights: AssetMLInsights = {
    regimeImpact: null,
    momentumShift: null,
    mlSignal: null,
    riskScore: null
 ...
 */
function analyzeRegimeImpact(result: AssetData, regimePrediction: RegimePredictionInput): RegimeImpactResult | null {
 ...
 */
function detectMomentumShiftML(result: AssetData, allAssets: AssetData[]): MomentumShiftResult {
  ...
  const allRoc6m = allAssets.map((a: AssetData) => parseFloat(a.details?.momentum?.roc6m as string) || 0);
  const percentile = allRoc6m.filter((r: number) => r < roc6m).length / allRoc6m.length;
 ...
 */
function generateMLSignal(result: AssetData, _marketData: MarketDataInput | null): MLSignalResult {
 ...
 */
function calculateMLRiskScore(result: AssetData, allAssets: AssetData[] | null): MLRiskScoreResult {
  const volatility = parseFloat(result.details!.risk.volatility) || 0;
  ...
  if (allAssets && allAssets.length > 0) {
    const allVolatilities = allAssets.map(a => parseFloat(a.details?.risk?.volatility as string) || 0);
...
export function filterByPriority(
  recommendations: any[], minPriority = RECOMMENDATION_PRIORITY.MEDIUM.level
) {
  return recommendations.filter((rec: any) => rec.priority.level >= minPriority);
}

/**
 * Group recommendations by type
 */
export function groupByType(recommendations: any[]) {
