...
declare global {
  interface Window {
    ...
    switchChartTab: typeof switchChartTab;
    clearAlertsLog: typeof clearAlertsLog;
    exportPortfolioExcel: typeof exportPortfolioExcel;
    exportAuditReport: typeof exportAuditReport;
    exportInvestmentCommitteeReport: typeof exportInvestmentCommitteeReport;
    exportClientReport: typeof exportClientReport;
    exportAttributionExcel: typeof exportAttributionExcel;
    exportAttributionPDF: typeof exportAttributionPDF;
    savePortfolioFromBuilder: () => Promise<void>;
...
let attributionDashboard: any = null;
 ...
 */
async function handlePortfolioSelection(event: Event) {
 ...
 */
export async function loadPortfolio(portfolioId: any) {
  try {
...
async function initializeAttributionDashboard() {
  ...
  try {
    ...
    section.innerHTML = `<div class="error-message" style="padding: 20px; color: #dc2626; background: #fee2e2; border-radius: 8px; margin: 10px;">Error loading attribution analysis: ${(error as Error).message}</div>`;
  }
}
...
async function buildAttributionData() {
  ...
  const benchmarkReturns = benchmarkPrices.map((p: any) => ({ date: p.date, value: p.price }));

  const attributionData: any = attributionAnalyzer.calculateAttribution(
    currentPortfolio,
    portfolioReturns,
    benchmarkReturns
  );
 ...
 */
function updateSummaryCards(pnlData: any, perfMetrics: any, benchmarkComparison: any) {
  // Total Value
  ...
  // Total Return
  const returnElement = document.getElementById('totalReturnCard') as HTMLElement;
  ...
  // Max Drawdown
  if (perfMetrics) {
    const ddElement = document.getElementById('maxDrawdownCard') as HTMLElement;
 ...
 */
function updatePositionsTable(positions: any) {
  ...
  tbody.innerHTML = positions.map((pos: any) => {
 ...
 * Update risk metrics
 */
function updateRiskMetrics(perfMetrics: any) {
...
async function updateRebalanceHistory() {
  ...
  container.innerHTML = history.slice(0, 5).map((r: any) => {
 ...
 */
async function checkAlerts(pnlData: any, perfMetrics: any, benchmarkComparison: any) {
  const alerts: any[] = [];
  ...
  if (perfMetrics && perfMetrics.max_drawdown.max_drawdown_pct <= thresholds.drawdown_pct) {
    ...
    await notifyRiskEvent({
      ...
      message: i18n.t('alerts.drawdown_message', {
        ...
        threshold: String(thresholds.drawdown_pct)
      }),
      metadata: {
  ...
  if (perfMetrics && perfMetrics.annualized_volatility_pct >= thresholds.volatility_pct) {
    ...
    await notifyRiskEvent({
      ...
      message: i18n.t('alerts.volatility_message', {
        volatility: perfMetrics.annualized_volatility_pct.toFixed(2),
        threshold: String(thresholds.volatility_pct)
      }),
      metadata: {
        portfolio_id: currentPortfolio?.id,
  ...
  if (pnlData.positions.length > 0) {
    const maxWeight = Math.max(...pnlData.positions.map((p: any) => p.weight));
    if (maxWeight > 25) {
      const position = pnlData.positions.find((p: any) => p.weight === maxWeight);
      alerts.push({
        type: 'warning',
  ...
  // Display alerts
  const alertsSection = document.getElementById('portfolioAlerts') as HTMLElement;
  const alertsContainer = document.getElementById('alertsContainer') as HTMLElement;

  if (alerts.length > 0) {
    alertsSection.style.display = 'block';
    alertsContainer.innerHTML = alerts.map((a: any) => {
...
async function loadAlertSettingsUI() {
  const strategyKey = currentPortfolio?.strategy || (document.getElementById('strategySelect') as HTMLSelectElement)?.value || 'balanced';
  const settings = await getAlertSettings(strategyKey);

  const volInput = document.getElementById('alertVolatilityThreshold') as HTMLInputElement;
  const ddInput = document.getElementById('alertDrawdownThreshold') as HTMLInputElement;
  const scoreInput = document.getElementById('alertScoreThreshold') as HTMLInputElement;
  const emailInput = document.getElementById('alertEmail') as HTMLInputElement;
  const webhookInput = document.getElementById('alertWebhook') as HTMLInputElement;
  const slackInput = document.getElementById('alertSlack') as HTMLInputElement;
  const teamsInput = document.getElementById('alertTeams') as HTMLInputElement;
  const zapierInput = document.getElementById('alertZapier') as HTMLInputElement;
  const strongSignalsToggle = document.getElementById('alertNotifySignals') as HTMLInputElement;
  const rebalanceToggle = document.getElementById('alertNotifyRebalances') as HTMLInputElement;
  const riskToggle = document.getElementById('alertNotifyRisk') as HTMLInputElement;

  if (volInput) volInput.value = String(settings.thresholds.volatility_pct);
  if (ddInput) ddInput.value = String(settings.thresholds.drawdown_pct);
  if (scoreInput) scoreInput.value = String(settings.thresholds.score);
...
async function handleAlertSettingsSave() {
  ...
  const volValue = Number.parseFloat((document.getElementById('alertVolatilityThreshold') as HTMLInputElement)?.value);
  const ddValue = Number.parseFloat((document.getElementById('alertDrawdownThreshold') as HTMLInputElement)?.value);
  const scoreValue = Number.parseFloat((document.getElementById('alertScoreThreshold') as HTMLInputElement)?.value);
  ...
  const updatedSettings = await saveAlertSettings({
    ...
    channels: {
      email: (document.getElementById('alertEmail') as HTMLInputElement)?.value || '',
      webhook: (document.getElementById('alertWebhook') as HTMLInputElement)?.value || '',
      slack: (document.getElementById('alertSlack') as HTMLInputElement)?.value || '',
      teams: (document.getElementById('alertTeams') as HTMLInputElement)?.value || '',
      zapier: (document.getElementById('alertZapier') as HTMLInputElement)?.value || ''
    },
    notifyOn: {
      strongSignals: (document.getElementById('alertNotifySignals') as HTMLInputElement)?.checked ?? true,
      rebalances: (document.getElementById('alertNotifyRebalances') as HTMLInputElement)?.checked ?? true,
      riskEvents: (document.getElementById('alertNotifyRisk') as HTMLInputElement)?.checked ?? true
    }
  });
...
async function loadAlertLogsUI() {
  ...
  container.innerHTML = alerts.map((alert: any) => {
    ...
    const channelTags = (alert.delivery_results || [])
      .map((result: any) => `<span style="padding: 2px 6px; border-radius: 6px; background: #334155; color: #e2e8f0; font-size: 0.75em;">${result.channel}</span>`)
      .join(' ');
 ...
 * Switch chart tab
 */
export function switchChartTab(tab: any) {
  currentChartTab = tab;

  // Update tab styles
  document.querySelectorAll('.chart-tab').forEach((btn: Element) => {
    const htmlBtn = btn as HTMLElement;
    if (htmlBtn.dataset.tab === tab) {
      htmlBtn.style.background = '#334155';
      htmlBtn.style.color = 'white';
      htmlBtn.classList.add('active');
    } else {
      htmlBtn.style.background = '#1e293b';
      htmlBtn.style.color = '#94a3b8';
 ...
 */
function updateChart(tab: any, data: any) {
 ...
 */
function createEquityCurveChart(ctx: any, equityCurve: any) {
  return new Chart(ctx, {
    type: 'line',
    data: {
      labels: equityCurve.map((p: any) => p.date),
      datasets: [{
 ...
 */
function createDrawdownChart(ctx: any, equityCurve: any) {
  const drawdowns = performanceTracker.calculateDrawdowns(equityCurve);

  return new Chart(ctx, {
    type: 'line',
    data: {
      labels: drawdowns.map((d: any) => d.date),
      datasets: [{
        label: i18n.t('portfolio_dashboard.drawdown'),
        data: drawdowns.map((d: any) => d.drawdown_pct),
        borderColor: '#ef4444',
 ...
 */
function createBenchmarkComparisonChart(ctx: any, equityCurve: any, benchmarkComparison: any) {
  if (!benchmarkComparison) {
      ...
      datasets: [
        {
          ...
          data: equityCurve.map((p: any) => p.return_pct),
          borderColor: '#10b981',
          backgroundColor: 'rgba(16, 185, 129, 0.1)',
          borderWidth: 2,
          fill: false,
        ...
        },
        {
          ...
          data: benchmarkComparison.benchmark_equity.map((p: any) => p.return_pct),
          borderColor: '#60a5fa',
 ...
 */
function createAllocationChart(ctx: any, positions: any) {
  const colors = [
  ...
  return new Chart(ctx, {
    type: 'doughnut',
    data: {
      labels: positions.map((p: any) => p.ticker),
      datasets: [{
        data: positions.map((p: any) => p.weight),
...
window.savePortfolioFromBuilder = async function() {
  ...
  try {
    ...
    (document.getElementById('portfolioSelector') as HTMLSelectElement)!.value = portfolio.id;
...
window.deleteCurrentPortfolio = async function() {
  ...
  try {
    ...
    await loadPortfolioList();
    (document.getElementById('portfolioSelector') as HTMLSelectElement)!.value = '';
    clearDashboard();
  ...
  try {
    ...
    const riskData = {
      ...
      concentration: currentPortfolio.positions?.length > 0
        ? Math.max(...currentPortfolio.positions.map((p: any) => p.weight || 0))
        : 0,
