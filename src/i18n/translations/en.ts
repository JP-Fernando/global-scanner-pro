...
export default {
  ...
  recommendation: {
    ...
    ml_anomaly_override: "🚨 ML ANOMALY ALERT: {ticker} has {count} ML anomalies with {severity} severity. Even with a Quant Score of {score}/100 (Short-term: {scoreShort}/100, Medium-term: {scoreMedium}/100, Long-term: {scoreLong}/100), this asset carries elevated risk. RECOMMENDATION (aggressive): You may keep interest or enter with a reduced, staged position, using stops and validating news/volume. RECOMMENDATION (balanced): Consider a partial entry and wait for anomaly normalization confirmation. RECOMMENDATION (conservative): Avoid increasing exposure until anomalies resolve; if already holding, reduce or hedge.",
