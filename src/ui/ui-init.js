...
export class UIInitializer {
  ...
  optimizeEventHandlers() {
    ...
    document.querySelectorAll('.scrollable, [data-scroll]').forEach(container => {
      const originalHandler = container.onscroll;
      if (originalHandler) {
        container.onscroll = throttle(originalHandler, 100);
      }
    });
    ...
    if (originalResize) {
      window.onresize = debounce(originalResize, 200);
    }
  }
