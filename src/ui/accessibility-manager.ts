...
export class AccessibilityManager {
    ...
    if (container) {
      ...
      if (!main) {
        ...
        if (controls) {
          ...
          if (parent) parent.appendChild(main);
        }
      }
    }
  ...
  handleModalTabTrap(e: KeyboardEvent, modal: Element): void {
    if (e.key !== 'Tab') return;
  ...
  enableArrowKeyNavigation(list: Element): void {
    const items = list.querySelectorAll<HTMLElement>('[role="option"], [role="menuitem"]');
    let currentIndex = 0;
    ...
    const observer = new MutationObserver((mutations) => {
      mutations.forEach(mutation => {
        mutation.addedNodes.forEach(node => {
          if (node.nodeType === 1 && (node as HTMLElement).classList?.contains('error')) {
            const errorField = (node as HTMLElement).querySelector('input, select, textarea') as HTMLElement | null;
            if (errorField) {
              errorField.focus();
  ...
  enhanceFormControls() {
    ...
    document.querySelectorAll('input, select, textarea').forEach(field => {
      field.addEventListener('invalid', (e) => {
        e.preventDefault();
        this.showFieldError(field as HTMLInputElement, (field as HTMLInputElement).validationMessage);
      });
    });
  }
